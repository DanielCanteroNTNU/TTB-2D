...
Train.Veh(veh_num).Body.m = 27122;                        % Main body mass [kg]
Train.Veh(veh_num).Body.I = 1254000;                      % Main body mass moment of inertia [kg*m^2]
Train.Veh(veh_num).Body.L_F = 5.448;                      % Distance from body's centre of gravity to front bogie centre [m]
Train.Veh(veh_num).Body.L_B = 13.252;                     % Distance from body's centre of gravity to front bogie centre [m]
Train.Veh(veh_num).Body.L = Train.Veh(veh_num).Body.L_F + Train.Veh(veh_num).Body.L_B;    % Main body length [m] (From bogie to bogie)
Train.Veh(veh_num).Body.Le = [1,1]*(21.965-Train.Veh(veh_num).Body.L);                % Additional distance [m], front and back
...
Train.Veh(veh_num).Susp.Prim.k = [1,1,1,1]*1320e3;        % Primary suspension stiffness [N/m]
Train.Veh(veh_num).Susp.Prim.c = [1,1,1,1]*12e3;          % Primary suspension viscous damping [Ns/m]
Train.Veh(veh_num).Susp.Sec.k = [1,1]*250e3;              % Secondary suspension stiffness [N/m]
Train.Veh(veh_num).Susp.Sec.c = [1,1]*20e3;               % Secondary suspension viscous damping [Ns/m]
